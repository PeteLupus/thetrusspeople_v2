...
export async function POST(request: NextRequest) {
    ...
    const phone = given(sheet.phone_number)
        ? `${sheet.phone_number}${sheet.phone_confirmed ? ' (read back and confirmed)' : ' (not confirmed)'}`
        : callerId ? `${callerId} (caller ID, not confirmed)` : 'no number taken';

    const rows: [string, string][] = [
        ['Caller', name + (sheet.company ? `, ${sheet.company}` : '')],
        ['Call back on', phone],
        ['Email', sheet.email || 'not given'],
    ...
    const subject = `${urgent ? 'URGENT: ' : ''}Victoria took a call: ${name}, ${wants}${sheet.suburb ? ` in ${sheet.suburb}` : ''}`;
    ...
    const html = `
        ...
        ${urgent ? '<p style="margin:0 0 16px;padding:8px 12px;background:#fde8e8;color:#8a1c1c;font-weight:bold">The caller flagged this as urgent.</p>' : ''}
    ...
    const replyTo = sheet.email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(sheet.email) ? { replyTo: sheet.email } : {};
