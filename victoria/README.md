...
| **Phone numbers are checked by a tool, not the model.** `check_phone_number` (tool `00319fe3`, server `thetrusspeople.com.au/api/victoria/tools`, same shared secret) normalises the caller's words, expands double and triple, validates Australian patterns and returns the exact read-back words. Haiku miscounted ten as nine and eleven as ten inside one call on 5 Sep | call 5 transcript, `app/api/victoria/tools/route.ts`, 11 unit cases Call six (5 Sep 21:36) proved the request reaches the route and exposed a parse bug: built to the docs example (flat name and arguments) while Vapi sends `function.name` and `function.arguments` as a JSON string. Fixed 23:41, proven on production with the call's own numbers; first live-call proof is still owed. Caller ID first, proven 6 Sep with a faked caller ID: when a number is showing she asks if it is the best one, a yes is confirmed with no digits spoken, and the sheet normalises +61 before judging. |
...
| One real call on record: the operator's 2 min 46 s test through the dashboard Talk button, 5 Sep 18:11 AEST. Every guardrail held, the sheet captured what he said, median reply gap 1.8 s measured off the recording, three misses logged in `v5-test-log.md` | `vapi.py call 01a0709f`, stereo recording By 21:36 on 5 Sep there are six: two through the Talk button, four on the published config, plus two dead attempts at 21:25 where the browser never sent the mic (test page fixed, see `v5-test-log.md`). |
